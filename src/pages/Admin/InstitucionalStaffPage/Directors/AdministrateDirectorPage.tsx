...
import { CREATE_DIRECTOR, UPDATE_USER } from "@/graphql/mutation/user.mutation";
...
import { DirectorFormDataSerializer } from "./DirectorFormDataSerializer";

const AdminDirectorPage = () => {
  const [createDirector] = useMutation(CREATE_DIRECTOR);
  const [updateDirector] = useMutation(UPDATE_USER);
  const [formLoading, setFormLoading] = useState(false);
  const [getDirectorData] = useLazyQuery(GET_USER_BY_ID_TO_UPDATE);
  ...
  const getDirectorFromServer = async (userId: number) => {
    try {
      ...
      const formData = new DirectorFormDataSerializer().directorDataToFormData(
  ...
  const handleSubmit = useCallback(
    async (values: ICreateUserFormDto) => {
      try {
        ...
        const input = new DirectorFormDataSerializer().formDataToCreateData(
          values
        );
        ...
        navigate("/app/institucionalStaff/directors");
      } catch (error) {
        console.error(error);
        setFormLoading(false);
      }
  ...
  return (
    <>
      <form onSubmit={form.onSubmit(handleSubmit)}>
        <Title order={2}>Registrar Director</Title>
        <Space h="sm" />
        <Title order={4}>Datos del Director:</Title>
        <Space h="sm" />
        <Grid
          sx={(theme) => ({
        ...
        <Divider></Divider>
        <Space h="sm" />
        <Title order={4}>Datos residenciales del director:</Title>
        <Space h="sm" />
        <Grid
          sx={(theme) => ({
            marginLeft: theme.spacing.xs,
